...
// New data packet format: counter | ax | ay | az | gx | gy | gz | 0x00 | 0x00
const int numSamples = 16;                     // Reduced samples per packet to fit new format
const int bytesPerSample = 10;                 // 9 values × 2 bytes each
...
// Function to pack data into the format: counter | ax | ay | az | 0x00
void packSensorData(uint8_t* buffer, int sampleIndex, uint16_t counter, int16_t ax, int16_t ay, int16_t az) {
  ...
  while (1) {
    if (isConnected) {
      ...
      for (int i = 0; i < numSamples; i++) {
        ...
        // Pack the data in the required format
        packSensorData(packetBuffer, i, packetCounter++, 
                       myData.xData, myData.yData, myData.zData);
      }
