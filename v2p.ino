...
uint16_t sampleCounter = 0;                    // Counter for individual samples
...
// Enable high sampling rate (12.8kHz) for connected mode
void enableHighSamplingRate() {
  kxAccel.enableAccel(false);
  kxAccel.setRange(SFE_KX134_RANGE64G);
  kxAccel.enableDataEngine();
  kxAccel.setOutputDataRate(13);  // Changed from 11 to 13 for 12.8KHz sampling rate
  kxAccel.enableAccel();
}
  ...
  while (1) {
    if (isConnected) {
      ...
      for (int i = 0; i < numSamples; i++) {
        ...
        // Pack the data in the required format (with PDM data and sample counter)
        packSensorData(packetBuffer, i, sampleCounter++, 
      ...
      if (sendBLEDataWithRetry(customCharacteristic, packetBuffer, sizeof(packetBuffer))) {
        ...
        packetCounter++; // Increment packet counter only on successful transmission
      } else {
